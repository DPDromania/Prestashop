...
class CartChecksum extends CartChecksumCore
{
    ...
    public function generateChecksum($cart)
    {
        ...
        $uniq_id .= sha1($cart->id_address_delivery); // $this->addressChecksum->generateChecksum(new Address($cart->id_address_delivery));
        $uniq_id .= $this->separator;
        $uniq_id .= sha1($cart->id_address_invoice); // $this->addressChecksum->generateChecksum(new Address($cart->id_address_invoice));
        $uniq_id .= $this->separator;
