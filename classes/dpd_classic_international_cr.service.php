...
class DpdGeopostCarrierClassicInternationalCRService extends DpdGeopostService
{
	const FILENAME = 'dpd_classic_international_cr.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASSIC_INTERNATIONAL_CR_ID);
		...
		$carrier_service = new DpdGeopostCarrierClassicInternationalCRService();

		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Classic International CR', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Classic International CR', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_CLASSIC_INTERNATIONAL_CR_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASSIC_INTERNATIONAL_CR_ID));
	}
}
