...
class DpdGeopostCarrierStandard24Service extends DpdGeopostService
{
	const FILENAME = 'dpd_standard_24.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_STANDARD_24_ID);
		...
		$carrier_service = new DpdGeopostCarrierStandard24Service();

		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Standard 24', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Standard 24', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_STANDARD_24_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_STANDARD_24_ID));
	}
}
