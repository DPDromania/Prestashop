...
class DpdGeopostCarrierPalletOneRomaniaService extends DpdGeopostService
{
	const FILENAME = 'dpd_pallet_one_romania.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_PALLET_ONE_ROMANIA_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierPalletOneRomaniaService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Pallet One Romania: ', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Pallet One Romania: ', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_PALLET_ONE_ROMANIA_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_PALLET_ONE_ROMANIA_ID));
	}
}
