...
class DpdGeopostCarrierBalkanService extends DpdGeopostService
{
	const FILENAME = 'dpd_balkan.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASSIC_BALKAN_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierBalkanService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Regional CEE', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Regional CEE', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_CLASSIC_BALKAN_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASSIC_BALKAN_ID));
	}
}
