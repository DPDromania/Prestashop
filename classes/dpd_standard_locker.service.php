...
class DpdGeopostCarrierStandardLockerService extends DpdGeopostService
{
	const FILENAME = 'dpd_standard_locker.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_DPD_LOCKER_ID);
		...
		$carrier_service = new DpdGeopostCarrierStandardLockerService();


		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Standard Locker', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Standard Locker', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_DPD_LOCKER_ID, (int)$carrier->id)) {
            return false;
        }
        return true;
	}
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_DPD_LOCKER_ID));
	}
}
