...
class DpdGeopostCarrierCargoRegionalService extends DpdGeopostService
{
	const FILENAME = 'dpd_cargo_regional.service';
	...
	public static function install()
	{
		...
		$carrier_service = new DpdGeopostCarrierCargoRegionalService();

		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Cargo Regional', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Cargo Regional', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_CARGO_REGIONAL_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_CARGO_REGIONAL_ID));
	}
}
