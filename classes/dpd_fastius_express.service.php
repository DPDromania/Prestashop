...
class DpdGeopostCarrierFastiusExpressService extends DpdGeopostService
{
	const FILENAME = 'dpd_fastius_express.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierFastiusExpressService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Fastius Express', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Fastius Express', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_ID));
	}
}
