...
class DpdGeopostCarrierLoccoCODService extends DpdGeopostService
{
	const FILENAME = 'dpd_locco_cod.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_COD_ID);
		...
		$carrier_locco_cod_service = new self();
		$carrier = new Carrier();
		$carrier->name = $carrier_locco_cod_service->module_instance->l('DPD Classic Locco + COD', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		$carrier->external_module_name = $carrier_locco_cod_service->module_instance->name;
		$carrier->url = _DPDGEOPOST_TRACKING_URL_;

		$delay = array();
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_cod_service->module_instance->l('DPD Classic Locco + COD', self::FILENAME);
		$carrier->delay = $delay;
		...
		foreach ($carrier_locco_cod_service->continents as $continent => $value)
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_LOCCO_COD_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_COD_ID));
	}
}
