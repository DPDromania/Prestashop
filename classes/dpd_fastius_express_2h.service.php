...
class DpdGeopostCarrierFastiusExpress2HService extends DpdGeopostService
{
	const FILENAME = 'dpd_fastius_express_2h.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_2H_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierFastiusExpress2HService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Fastius Express 2H', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		$carrier->external_module_name = $carrier_locco_service->module_instance->name;
		$carrier->url = _DPDGEOPOST_TRACKING_URL_;

		$delay = array();
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Fastius Express 2H', self::FILENAME);
		$carrier->delay = $delay;
		...
		foreach ($carrier_locco_service->continents as $continent => $value)
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_2H_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_FASTIUS_EXPRESS_2H_ID));
	}
}
