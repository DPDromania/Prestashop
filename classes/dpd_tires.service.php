...
class DpdGeopostTiresService extends DpdGeopostService
{
	const FILENAME = 'dpd_tires.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_TIERS_ID);
		...
		$carrier_locco_service = new DpdGeopostTiresService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD TIRES', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD TIRES', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_TIERS_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_TIERS_ID));
	}
}
