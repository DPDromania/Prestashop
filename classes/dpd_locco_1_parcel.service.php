...
class DpdGeopostCarrierLocco1ParcelService extends DpdGeopostService
{
	const FILENAME = 'dpd_locco_1_parcel.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_1_PARCEL_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierLocco1ParcelService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Classic Locco 1 Parcel', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Classic Locco 1 parcel', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_LOCCO_1_PARCEL_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_1_PARCEL_ID));
	}
}
