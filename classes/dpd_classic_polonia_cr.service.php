...
class DpdGeopostCarrierClassicPoloniaCRService extends DpdGeopostService
{
	const FILENAME = 'dpd_classic_polonia_cr.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASIC_POLONIA_CR_ID);
		...
		$carrier_service = new DpdGeopostCarrierClassicPoloniaCRService();

		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Classic Polonia CR', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Classic Polonia CR', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_CLASIC_POLONIA_CR_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_CLASIC_POLONIA_CR_ID));
	}
}
