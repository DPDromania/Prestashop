...
class DpdGeopostCarrierCargoNationalService extends DpdGeopostService
{
	const FILENAME = 'dpd_cargo_national.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_CARGO_NATIONAL_ID);
		...
		$carrier_service = new DpdGeopostCarrierCargoNationalService();

		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD Cargo National', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD Cargo National', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_CARGO_NATIONAL_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_CARGO_NATIONAL_ID));
	}
}
