...
class DpdGeopostCarrierLoccoService extends DpdGeopostService
{
	const FILENAME = 'dpd_locco.service';
	...
	{
		$id_carrier = (int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_ID);
		...
		$carrier_locco_service = new DpdGeopostCarrierLoccoService();

		$carrier = new Carrier();
		$carrier->name = $carrier_locco_service->module_instance->l('DPD Classic Locco', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_locco_service->module_instance->l('DPD Classic Locco', self::FILENAME);
		$carrier->delay = $delay;
		...
		if (!Configuration::updateValue(DpdGeopostConfiguration::CARRIER_LOCCO_ID, (int)$carrier->id))
			return false;
	...
	{
		return (bool)self::deleteCarrier((int)Configuration::get(DpdGeopostConfiguration::CARRIER_LOCCO_ID));
	}
}
